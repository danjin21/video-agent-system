...
## ⚙️ 시스템 오버라이드 (우리 영상 시스템 — 사용자 명시 승인 2026-06-26)

이 문서는 HP 디자인 언어의 정본 참조다. 단, **영상 슬라이드 시스템에 맞춰 아래 2가지를 의도적으로 오버라이드**한다. 충돌 시 이 블록이 우선한다.

1. **디스플레이/히어로 weight: 500 → 600.** HP 원본은 히어로 72px도 weight 500을 고수하지만, 우리 시스템은 더 또렷한 **600**을 기본으로 쓴다. 정말 강한 임팩트 헤드라인·핵심 수치는 **700까지** 허용. **800/900은 금지.** (본문 400, 버튼 600은 HP 그대로.)
2. **다크 배경 폐기 → 화이트/블루 2면.** HP 원본은 다크 네이비 슬랩(`#1a1a1a`)으로 클로징·테스티모니얼 밴드를 닫지만, 우리는 **다크 배경을 쓰지 않는다.** 클로징·간지·표지의 강조 면은 **화이트(`#ffffff`) 버전** 또는 **블루(`#024ad8`) 풀블리드 버전(흰 텍스트)** 둘 중에서만 고른다. 따라서 아래 본문의 `help-band-dark`, `footer-dark`, `promo-strip-dark`의 "dark navy slab"은 모두 **블루 풀블리드**로 읽는다. (`#1a1a1a`는 텍스트 잉크 색으로만 유지.)

폰트는 Forma DJR Micro 대신 **Pretendard**(국문+영문/숫자) 사용. 그 외 컬러·radius·spacing·셰브론·카드 규칙은 HP 원본 그대로.

### 📹 영상 전용 규칙 (최종 결과물은 pptx가 아니라 영상)

이 시스템의 산출물은 **1920×1080 영상**이다. HP 원본은 웹 페이지 스크롤 시스템이지만, 우리는 슬라이드 + 모션으로 번역한다.

**레이아웃 (중앙축 구성)**
- 대부분의 콘텐츠는 **가운데**. **상단 타이틀(+kicker) → 중앙 메인 콘텐츠 → 하단 블루 배너(상황별)** 3단.
- **차트·그래프·표는 x축뿐 아니라 y축(수직)도 중앙 정렬.** 메인 콘텐츠는 *헤더 아래 ~ 하단 배너 위* 의 콘텐츠 존 안에서 **수직 가운데**에 놓는다. 차트를 위로 붙여 **타이틀을 침범하거나**, 아래로 붙여 배너와 겹치게 하지 말 것. (콘텐츠 존 = 대략 y 230~950, 배너 사용 시 배너 높이만큼 위로 좁힘.)
- **하단 블루 배너**: 길고 두꺼운 `#024ad8` 풀블리드 띠. 바로 위 장표에 대한 코멘트/결과를 **한 줄**로 (흰 텍스트, weight 600). 등장은 **pop(0.92→1.06→1.0 살짝 커졌다 안착) + 흰 shine 스윕 1회**로 눈에 띄게 — 단순 페이드 금지. 항상은 아니고 필요할 때만.
- **좌상단 로고는 슬라이드(Remotion)에 직접 박지 않는다.** `top_left`는 비워두고 **로고는 Premiere 전역 오버레이**로 얹는다. (텍스트 워드마크도 슬라이드에 렌더하지 말 것 — 좌상단은 항상 비움.)
- **강조 키워드 대형 reveal**: 결정적 키워드(예: "AX", 핵심 수치)는 표·콘텐츠를 비우고 **거의 화면 가득 대형 텍스트**로, 글자 단위 타이밍에 맞춰 딱딱 등장(A → X 스태거). 작은 배너/캡션으로 흘리지 말 것.
- **양옆 구도는 동시에 깔지 않는다.** 좌 그래프 + 우 표 같은 경우: ① 그래프가 **가운데** 등장+애니메이션 → ② 표 차례에 그래프가 **왼쪽으로 슬라이드** 이동하고 우측에 표 페이드인+애니메이션.

**모션 — 2종 분리 (2026-06 정적 슬라이드 방지)**
- **리빌 모션**(키워드·카드·라벨·간지 *등장*): **완결 ≤ 1.5초 후 정지 hold.** 등장은 빠르게 끝내고 멈춘다.
- **의미화 모션**(숫자 카운트업, 원/막대 성장, 게이지 채움, 곡선 draw 등 *값·논점 전개*): **그 내용을 말하는 발화 비트 길이에 맞춰 전개**하고 **수치가 발화되는 순간 정착**(예: "501조" 말할 때 501 도달). 1.5초로 자르지 말 것. 정착 후 hold. 한 슬라이드에 의미화 모션 주인공은 1개(둘이면 시간차 페이즈).
  - 큰 숫자는 그냥 띄우지 말고 **카운트업+성장 원/막대**(예: 432조→501조). 은유(예: "기울어진 운동장")는 아이콘/일러스트 모션이나 b-roll. 공용 컴포넌트 `Dynamics.tsx`(CountUp/GrowCircle/TiltField).
- **🚫 긴 내레이션 위 정적 금지**: >8초 비트 위에서 슬라이드가 ≤1.5초 만에 다 멈추면 안 됨 — 의미화 모션 배치 또는 비트 잘게 쪼개 점진 노출.
- **idle 정지**: 기본 상태에서 둥둥 떠다님·미세 흔들림·ambient 루프 **전부 금지.** 자리잡으면 멈춘다. (의미화 모션은 무한 루프가 아닌 발화 동기 1회 전개이므로 예외)
- **트리거 = 내레이션 비트 동기**: 내레이터가 해당 내용을 말하는 순간 요소 등장 (continuity가 Supertone WAV duration 기반으로 프레임 배치).
- **슬라이드 전환 = 컷** (하드컷, 디졸브/와이프 없음). 내부 요소 애니메이션이 모션 담당.
- **끝 대기(trailing hold)**: 중간 슬라이드는 마지막 콘텐츠/오디오 후 **~1.5초**만. **5초 end-hold는 영상 전체 마지막 슬라이드에만** — 중간을 길게 비우면 죽은 시간.

**비주얼 — 다이어그램 우선 (2026-06 사용자 핵심: "도형이 많이 나와야 한다")**
- **개념은 글자가 아니라 도형으로.** 메시지/개념 슬라이드도 텍스트만 두지 말고 노드·원·커넥터·아이콘·칩 **다이어그램**으로. 영상이 정적으로 느껴지는 1순위 원인은 모션이 아니라 화면에 볼 게 없는 것(시각적 빈약함).
- **은유는 픽토그램 도형**: 저수지=분지+물방울+수위, 운동장=구르는 공+좁아지는 바닥, 두 근육=원 2개+링크. 글자로만 두지 말 것.
- **아이콘은 청중이 1초 안에 읽는 의미로**: 수익률=`%`, AI/AX=로봇, 성장=상승 화살표, 자금=물방울. 빗나간/추상 메타포 금지(예: AX=반도체 칩 ✕).
- **강조 키워드 = 대형 고스트 타이포**: 결정타 숫자·단어("10년", "사상 최대")를 콘텐츠 뒤·옆에 *크게 옅게*(opacity ~0.08) 한 번 더, 그 단어 발화 순간 등장. 슬라이드당 1회.
- **역동성 = 빠른 여러 도형**(느린 단일 트윈/8초 게이지 크롤 ✕). 다이어그램 요소를 빠른 pop(≤0.7s)으로 연달아.

**카피 (영상이라 더 엄격)**
- **내레이션 자막 표시 안 함** — 내레이션은 오디오로만 나간다.
- **장표 텍스트에 문장형(`~다 / ~합니다 / ~했다`) 절대 금지.** 타이틀/키워드/숫자/라벨/**체언 종결**만. 중(中)타이틀·설명 문장 금지. (내레이션이 "~다"로 끝나므로 화면에 같은 문장형이 뜨면 중복이고 격이 떨어진다.)
- 하단 배너의 결과·코멘트도 **헤드라인형 한 줄**(체언 종결). 예: ⭕ "3년 연속 업계 1위" / ❌ "3년 연속 업계 1위를 기록했습니다".
