...
정본 `DESIGN.md`(HP)가 데이터/타이틀 중심이라면, 이 가이드는 **실사진/로고 콜라주 + 짧은 라벨** 중심이다.
...
// Sub-kicker (중간 강조, HP Electric Blue)
sub_kicker:
  size_px: 26-30
  weight: 500
  color: #024ad8
...
// Main title (큰 한국어 제목, near-black ink)
main_title:
  size_px: 80-92
  weight: 500
  color: #1a1a1a
...
정본 `DESIGN.md` (HP) 컬러 시스템 그대로 사용. 추가로:

```
card_background: #ffffff
card_radius_px: 16            // HP 카드 정본 (xl)
card_shadow: 0 2px 8px rgba(26, 26, 26, 0.08)   // HP Soft Lift
card_border: 1px solid #e8e8e8                  // HP hairline
connecting_line_color: #296ef9 (또는 #024ad8 50% opacity)
...
cards: 4개 (또는 3-6개 자유)
  ...
  card_internal:
    ...
    label_area: 카드 하단 ~30%
      brand_name: 16-18px, weight 500, color #1a1a1a (좌)
      tag_line:   12-14px, weight 400, color #636363 (우)

connecting_lines:
  type: SVG cubic bezier
  color: #296ef9 alpha 0.6
  stroke_width: 2-3
...
- "코닥" 발화 → Kodak 카드 등장
- "노키아" → Nokia 카드 등장
- 카드 등장이 발화 타이밍에 동기화 (baseline `motion.trigger` 내레이션 비트 동기 / continuity의 audio boundary 룰, 각 등장 ≤1.5초)
...
3. 배경: 플랫 화이트(#ffffff) 또는 블루(#024ad8) 풀블리드 (HP 플랫 원칙 — gradient 금지)
4. 3-6개 카드 유기적 배치
5. 곡선 연결선 SVG
6. 카드별 등장 스태거
