...
- **⭐ 모션 2종 분리 (2026-06, 정적 슬라이드 방지의 핵심)** — 모션을 두 종류로 구분하고 규칙을 달리 적용한다:
  ...
  - **의미화 모션** (숫자 카운트업, 원/막대 성장, 게이지 채움, 곡선 draw 등 *값·논점이 전개되는* 모션): **그 내용을 말하는 발화 비트의 길이에 맞춰 전개**한다. 1.5초로 자르지 말 것 — 비트가 6초면 카운트업도 그 안에서 진행하다 **수치가 발화되는 순간 정착**(예: "501조로"라고 말할 때 501에 도달). 정착 후 남는 시간은 hold.
...
- **긴 문장은 모션을 문장에 펼친다**: 의미화 모션(카운트업·게이지·곡선 draw·메타포 변형)의 `interpolate` 구간 끝을 **그 문장의 핵심어가 발화되는 프레임**에 맞춘다. 문장이 길면 ① 도입 → ② 전개 → ③ 핵심어에서 정착, 의 다단계로 모션을 문장 전체에 분배한다.
- **메타포는 한 번 변형 후 멈추지 말고 문장 내내 진행형으로**: 예) "좁아지는 운동장" 문장 = 운동장 너비가 문장 발화 내내 *계속* 줄어든다(markers 압축), 공 정지 X. "쌓이는 돈" = 원/막대가 문장 내내 차오른다.
- **콘티 산출물에 문장(beat)별 action 칸 필수**: 각 beat에 `{beat, text(문장), dur_s, action(그 문장 동안 무엇이 어떻게 움직이는가)}`를 *전 문장 빠짐없이* 적는다. action이 비거나 "정지"인 beat가 있으면 콘티 미완성으로 간주.
- **문장 안에 변화가 없으면 만들어낸다**: 순수 메시지 문장이라도 키워드 밑줄 스윕/색 강조 이동/카드 미세 등장 등 *발화에 동기화된* 1회 진행을 둔다(단 idle 루프·둥둥 금지). 단, 과하면 절제 — "작은 진행 하나"면 충분.
