...
- **빈 `TBD`로 두지 말 것 (2026-06 사용자 요구).** ① **공개 데이터**(ETF·퇴직연금·시장통계·미국시장 등)는 **실제 웹서치로 수치를 채운다**. ② **내부·비공개 수치**(상반기 매출·세전이익·부문 점유율 등 공시 전/비공개)는 웹으로 못 구하므로 **그럴듯한 근사치(estimate)를 넣되 빨간 `(*) TBD 수정 필요`** 로 표기 — 자리만 비운 `[TBD]` 박스 금지. 근사치엔 `status:"estimate_placeholder"` + 근거(전년/업계 추정)를 註로.
