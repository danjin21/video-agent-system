...
## 🎞️ 정적 방지 — 의미화 모션 & 메타포 비주얼 (2026-06)

**비트별로 판단**하되(혼합), 절제는 유지한다:

- **큰 숫자/지표** → 그냥 띄우지 말고 **카운트업 + 성장 원/막대**(Remotion `Dynamics.tsx`의 `CountUp`/`GrowCircle`). 수치가 발화되는 순간 정착(예: "501조" 말할 때 501 도달). continuity의 '의미화 모션' 규칙(발화 길이에 맞춰 전개, ≤1.5초로 자르지 않음).
- **비교/변화** → 게이지 2단, 곡선 lift/역전 등.
